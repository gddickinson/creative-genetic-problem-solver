...
class GameEvolutionVisualizer:
    ...
    def evolve_population(self):
        ...
        for generation in range(self.n_generations):
            ...
            payoffs = payoffs - np.min(payoffs)  # Make all payoffs non-negative
            payoffs = payoffs + 1e-10  # Add small constant to avoid zero probabilities
            ...
            reproduction_probs = payoffs / np.sum(payoffs)  # Now guaranteed to be valid probabilities
            ...
            for _ in range(self.population_size):
                ...
                if random.random() < parent.mutation_rate:
                    cooperate_prob = np.clip(
                        ...
                        0.0, 1.0  # Ensure probability stays in valid range
                    )
                child = Strategy(
                    parent.name,
                    cooperate_prob,
    ...
    def update_plot(self, frame: int):
        ...
        for i, strategy_name in enumerate(self.strategies.keys()):
            # Extract history for this strategy
    ...
    def animate(self, game_type: str = 'prisoners_dilemma'):
        ...
        for strategy, proportion in final_dist.items():
            ...
            if percentage >= 1:  # Only show strategies that survived
        ...
        final_performance = self.history[-1][1]
        print("\nFinal Strategy Performance:")
        for strategy, score in final_performance.items():
            if final_dist[strategy] > 0.01:  # Only show active strategies
                print(f"- {strategy}: {score:.2f}")
        
        # Analyze evolution patterns
        print("\nEvolution Analysis:")
        
        # Check for strategy extinction
        extinct = [strat for strat, prop in final_dist.items() if prop < 0.01]
        if extinct:
            print("Extinct Strategies:", ", ".join(extinct))
        
        # Check for stable coexistence
        coexisting = [strat for strat, prop in final_dist.items() if prop >= 0.1]
        if len(coexisting) > 1:
            print("Stable Coexistence between:", ", ".join(coexisting))
        
        # Game-specific insights
        if game_type == 'prisoners_dilemma':
            if final_dist['Cooperative'] > 0.6:
                print("\nCooperation emerged as a successful strategy!")
            elif final_dist['Defective'] > 0.6:
                print("\nDefection dominated the population.")
            else:
                print("\nMixed strategies prevailed.")
                
        elif game_type == 'hawk_dove':
            if len(coexisting) > 1:
                print("\nA balanced ecosystem evolved with multiple strategies.")
            else:
                print("\nOne strategy dominated the ecosystem.")
                
        elif game_type == 'stag_hunt':
            if final_dist['Cooperative'] > 0.6:
                print("\nThe population learned to coordinate and hunt stag together!")
            else:
                print("\nThe population failed to establish mutual trust.")
...
if __name__ == "__main__":
    ...
    print("- Cooperative (usually cooperates)")
    print("- Defective (usually defects)")
    print("- Tit-for-Tat (reciprocates opponent's last move)")
    print("- Grudger (cooperates until opponent defects, then always defects)")
